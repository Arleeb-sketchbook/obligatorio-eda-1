...
int main() {
    ...
    do {
        ...
        switch (opcion) {
                    ...
                    switch (subopcion) {
                        ...
                        case 2: {
                            if (estacionamiento->tope == -1) {
                                ...
                                int tarifah = (autoSaliente.hora_Salida - autoSaliente.hora_llegada)*50;
                                printf("A pagar:%d",tarifah);
            ...
            case 4:{
                char matricula_busqueda[8];
                printf("Ingrese la matrícula a buscar (7 caracteres): ");
                scanf("%7s", matricula_busqueda);

                int encontrado = 0;

                // Buscar en estacionamiento
                for (int i = 0; i <= estacionamiento->tope; i++) {
                    if (strcmp(estacionamiento->datos[i].matricula, matricula_busqueda) == 0) {
                        printf("Vehículo encontrado en el ESTACIONAMIENTO:\n");
                        mostrarAuto(estacionamiento->datos[i]);
                        encontrado = 1;
                    }
                }
                ...
                // Buscar en cola
                for (int i = 0; i < cola.tamaño; i++) {
                    int idx = (cola.frente + i) % cola.capacidad;
                    if (strcmp(cola.autos[idx].matricula, matricula_busqueda) == 0) {
                        printf("Vehículo encontrado en la COLA DE ESPERA:\n");
                        mostrarAuto(cola.autos[idx]);
                        encontrado = 1;
                    }
                }
                ...
                // Buscar en historial
                for (int i = 0; i < historial.cantidad; i++) {
                    if (strcmp(historial.autos[i].matricula, matricula_busqueda) == 0) {
                        printf("Vehículo encontrado en el HISTORIAL:\n");
                        mostrarAuto(historial.autos[i]);
                        encontrado = 1;
                    }
                }
                ...
                if (!encontrado) {
                    printf("No se encontró ningún vehículo con esa matrícula.\n");
                }
                break;
            }
